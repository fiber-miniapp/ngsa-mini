...
#ifdef _TARGET_KEI
		int op = getuint32(bam1_cigar(b)+k) & BAM_CIGAR_MASK; // operation
		int l = getuint32(bam1_cigar(b)+k) >> BAM_CIGAR_SHIFT; // length
#else
...
#ifdef _TARGET_KEI
						uint32_t cigar = getuint32(bam1_cigar(b)+i);
#else
...
#ifdef _TARGET_KEI
						uint32_t cigar = getuint32(bam1_cigar(b)+k+1); // next CIGAR
#else
...
#ifdef _TARGET_KEI
							cigar = getuint32(bam1_cigar(b)+k+2); op_next = cigar&BAM_CIGAR_MASK;
#else
