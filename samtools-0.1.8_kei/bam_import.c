	...
	{ // cigar
		...
		if (str->s[0] != '*') {
			...
			for (i = 0, s = str->s; i != c->n_cigar; ++i) {
				...
				s = t + 1;
#ifdef _TARGET_KEI
				putuint32(bam1_cigar(b)+i, x << BAM_CIGAR_SHIFT | op);
#else
