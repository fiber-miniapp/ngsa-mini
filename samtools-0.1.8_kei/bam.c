...
#ifdef _TARGET_KEI
		int op = getuint32(cigar+k) & BAM_CIGAR_MASK;
#else
		int op = cigar[k] & BAM_CIGAR_MASK;
#endif
		if (op == BAM_CMATCH || op == BAM_CDEL || op == BAM_CREF_SKIP)
#ifdef _TARGET_KEI
			end += getuint32(cigar+k) >> BAM_CIGAR_SHIFT;
#else
...
#ifdef _TARGET_KEI
			l += getuint32(cigar+k) >> BAM_CIGAR_SHIFT;
#else
...
#ifdef _TARGET_KEI
			kputw(getuint32(bam1_cigar(b)+i)>>BAM_CIGAR_SHIFT, &str);
			kputc("MIDNSHP"[getuint32(bam1_cigar(b)+i)&BAM_CIGAR_MASK], &str);
#else
...
#ifdef _TARGET_KEI
		else if (type == 'S') { kputsn("i:", 2, &str); kputw(getuint16(s), &str); s += 2; }
		else if (type == 's') { kputsn("i:", 2, &str); kputw(getint16(s), &str); s += 2; }
		else if (type == 'I') { kputsn("i:", 2, &str); kputuw(getuint32(s), &str); s += 4; }
		else if (type == 'i') { kputsn("i:", 2, &str); kputw(getint32(s), &str); s += 4; }
		else if (type == 'f') { ksprintf(&str, "f:%g", getfloat(s)); s += 4; }
		else if (type == 'd') { ksprintf(&str, "d:%lg", getdouble(s)); s += 8; }
#else
